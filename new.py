...
if uploaded_file:
    ...
    if time_column not in data.columns:
        ...
        if numeric_columns:
            selected_metric = st.selectbox("Select a Metric to View Trends", numeric_columns)
            fig = px.line(data, x=time_column, y=selected_metric, title=f"Trend: {selected_metric}")
            st.plotly_chart(fig)
        else:
        ...
        if "failure_probability" in data.columns:
            ...
            if not alerts.empty:
                st.warning("⚠ Potential issues detected in the following records:")
                st.write(alerts)
            ...
            st.error("Column 'failure_probability' not found in the data. Cannot generate alerts.")
...
input_method = st.sidebar.radio("Select Input Method", ("Manual Input", "Upload Dataset"))

# Single Equipment Input
if input_method == "Manual Input":
    st.subheader("Enter Sensor Readings")
    vibration = st.number_input("Vibration Level", min_value=0.0, max_value=100.0, value=50.0)
    temperature = st.number_input("Temperature (°C)", min_value=-10.0, max_value=150.0, value=30.0)
    humidity = st.number_input("Humidity (%)", min_value=0.0, max_value=100.0, value=50.0)
    pressure = st.number_input("Pressure (kPa)", min_value=0.0, max_value=200.0, value=100.0)
...
# Batch Input via Dataset
    ...
    if uploaded_file is not None:
        ...
        if all(col in input_df.columns for col in required_columns):
            ...
            input_df["Status"] = ["Faulty" if pred == 1 else "Healthy" for pred in predictions]
            
            # Display predictions
            st.subheader("Predicted Equipment Status")
            st.dataframe(input_df[["vibration", "temperature", "humidity", "pressure", "Status"]])
...
humidity_filter = st.sidebar.slider("Filter Humidity:", min_value=float(pd1["humidity"].min()), max_value=float(pd1["humidity"].max()), value=(float(pd1["humidity"].min()), float(pd1["humidity"].max())))
pressure_filter = st.sidebar.slider("Filter Pressure:", min_value=float(pd1["pressure"].min()), max_value=float(pd1["pressure"].max()), value=(float(pd1["pressure"].min()), float(pd1["pressure"].max())))
...
fig_plotly = px.scatter(filtered_df, x=x_axis, y=y_axis, color="humidity", size="pressure", title=f"{y_axis} vs {x_axis} (Interactive)")
